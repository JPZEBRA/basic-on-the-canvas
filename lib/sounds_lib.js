...
/* Version 1.100 / Revised 2025.03.03    */
/* Author : K-ARAI                       */
...
function gplib_sound_check(n) {

    var s = Math.floor(n);

    if(0 <= s && s < gplib_sound_count ) {
        if(gplib_sounds[s].duration > 0) return 0;
    }
...
function gplib_sound_state(n) {
    ...
    if(0 <= s && s < gplib_sound_count ) {
        if(!gplib_sounds[s].duration > 0) return -1;
...
function SOUND_CHECK(n) {

    return gplib_sound_check(n-1);
